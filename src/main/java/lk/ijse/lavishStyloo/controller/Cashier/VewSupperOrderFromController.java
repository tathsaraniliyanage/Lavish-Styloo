...
import lk.ijse.lavishStyloo.model.SupperOrderDetailsModel;
import lk.ijse.lavishStyloo.model.SupperOrderModel;
...
public class VewSupperOrderFromController implements Initializable {
    ...
    private void setOderDetails(String supperOrderId) {
        try {
            List<SupperOrderDetailsTm> details = SupperOrderDetailsModel.findCustomerOrderDetailsByOrderId(supperOrderId);
    ...
    private void loadSelectedDate() {
        ...
        try {
            List<SupperOrderTm> supperOrderTms = SupperOrderModel.findCustomerOrdersByDate(date);
    ...
    private void loadToDayOrders() {
        try {
            List<SupperOrderTm> supperOrderTms = SupperOrderModel.findCustomerOrdersByDate(DateTimeUtil.dateNow());
            if (supperOrderTms.get(supperOrderTms.size() - 1).getSupperOrderId() != null) {
                toProcess(supperOrderTms);
            } else {
                list.clear();
    ...
    private void loadAllOrders() {
        try {
            List<SupperOrderTm> supperOrderTms = SupperOrderModel.findAll();
            toProcess(supperOrderTms);
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
    ...
    private void setCount() {
        try {
            txtAllOrders.setText(SupperOrderModel.CountCustomerOrder() + "+");
            txtTodayOrdersCount.setText(SupperOrderModel.CountCustomerOrderByDate() + "+");
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
