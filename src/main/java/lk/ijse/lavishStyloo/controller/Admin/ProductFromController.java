...
public class ProductFromController implements Initializable {
    ...
    public void productFromSearchOnKeyReleased(KeyEvent keyEvent) {
        try {
            List<ProductDTO> product = ProductModel.findProductByLike(txtSearchText.getText());
            setTm(product);
    ...
    private void setPriceRange() {
        try {
            String min = ProductModel.MinProductByPrice();
            String max = ProductModel.MaxProductByPrice();
            txtMax.setText(max);
            txtMin.setText(min);
    ...
    public void loadAllProduct() {
        try {
            List<ProductDTO> product = ProductModel.findProduct();
            setTm(product);
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
    ...
    public void priceOnDrag(MouseEvent mouseEvent) {
        ...
        try {
            List<ProductDTO> product = ProductModel.betweenProductByPrice(format);
            setTm(product);
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
