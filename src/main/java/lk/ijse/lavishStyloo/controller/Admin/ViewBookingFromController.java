...
public class ViewBookingFromController implements Initializable {
    ...
    private void setBookingDetails(String bookingId) {
        try {
            List<BookingDetailsTm> list = BookingModel.findBookingDetailsByBookingId(bookingId);
    ...
    private void loadSelectedDate() {
        ...
        try {
            List<BookingTM> bookingTMList = BookingModel.findBookingByDate(date);
    ...
    private void loadToDayOrders() {
        try {
            List<BookingTM> bookingTMList = BookingModel.findBookingByDate(DateTimeUtil.dateNow());
    ...
    private void loadAllBooking() {
        try {
            List<BookingTM> bookingTMList = BookingModel.findBooking();
            toProcess(bookingTMList);
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
    ...
    private void setCount() {
        try {
            txtAllBookings.setText(BookingModel.CountBooking() + "+");
            txtTodayBookingsCount.setText(BookingModel.CountBookingByDate(DateTimeUtil.dateNow()) + "+");
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
