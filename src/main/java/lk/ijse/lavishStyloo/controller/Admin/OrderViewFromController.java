...
import lk.ijse.lavishStyloo.model.CustomerOrderModel;
...
public class OrderViewFromController implements Initializable {
    ...
    private static OrderViewFromController controller;

    public OrderViewFromController() {
        controller=this;
    }

    public static OrderViewFromController getController() {
        return controller;
    }
    ...
    private void setOderDetails(String customerOrderId) {
        try {
            List<CustomerOrderDetailsTm> customerOrders = CustomerOrderModel.findCustomerOrderDetailsByOrderId(customerOrderId);
    ...
    private void loadSelectedDate() {
        ...
        try {
            List<CustomerOrderTm> customerOrders = CustomerOrderModel.findCustomerOrdersByDate(date);
    ...
    private void loadToDayOrders() {
        try {
            List<CustomerOrderTm> customerOrders = CustomerOrderModel.findCustomerOrdersByDate(DateTimeUtil.dateNow());
            if (customerOrders.get(customerOrders.size() - 1).getCustomerOrderId() != null) {
                toProcess(customerOrders);
            } else {
                list.clear();
    ...
    private void loadAllOrders() {
        try {
            List<CustomerOrderTm> customerOrders = CustomerOrderModel.findCustomerOrders();
            toProcess(customerOrders);
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
    ...
    private void setCount() {
        try {
            txtAllOrders.setText(CustomerOrderModel.CountCustomerOrder() + "+");
            txtTodayOrdersCount.setText(CustomerOrderModel.CountCustomerOrderByDate() + "+");
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
