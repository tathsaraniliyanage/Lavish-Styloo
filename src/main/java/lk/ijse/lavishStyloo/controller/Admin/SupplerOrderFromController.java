...
public class SupplerOrderFromController implements Initializable {
    ...
    public void cmbSupperOnaAction(ActionEvent actionEvent) throws ClassNotFoundException {
        try {
            SupplierDTO supplierDTO = SupplierModel.findSupplierById(String.valueOf(cmbSupperId.getValue()));
    ...
    public void btnTotalOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean savedOrder = SupperOrderModel.placeOrder(list, supperOrderDTO);
            if (savedOrder){
    ...
    private String getNext() {
        try {
            return SupperOrderModel.nextId();
        } catch (SQLException | ClassNotFoundException throwables) {
            throwables.printStackTrace();
        }
        return null;
