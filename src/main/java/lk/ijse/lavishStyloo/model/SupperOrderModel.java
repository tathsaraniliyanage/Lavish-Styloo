...
public class SupperOrderModel {
    public static boolean placeOrder(ArrayList<OrderTm> list, SupperOrderDTO supperOrderDTO) throws SQLException, ClassNotFoundException {

        Connection connection = DBConnection.getInstance().getConnection();
        connection.setAutoCommit(false);
        try {
            boolean isSaved = setOrder(supperOrderDTO);
            if (isSaved) {
                boolean isSavedDetails = setOrderDetails(list, supperOrderDTO);
                if (isSavedDetails) {
                    boolean isUpdated = ProductModel.updateSupperItems(list);
                    if (isUpdated) {
                        return true;
                    } else {
                        connection.rollback();
    ...
    private static boolean setOrderDetails(ArrayList<OrderTm> list, SupperOrderDTO supperOrderDTO) throws SQLException, ClassNotFoundException {
        for (OrderTm d : list) {
            boolean isSaved = CrudUtil.crudUtil("INSERT INTO supplier_order_details VALUES (?,?,?,?)",
    ...
    private static boolean setOrder(SupperOrderDTO supperOrderDTO) throws SQLException, ClassNotFoundException {
        String sql = "INSERT INTO supplier_order VALUES (?,?,?,?,?)";
        return CrudUtil.crudUtil(sql,
                supperOrderDTO.getSup_oid(),
                supperOrderDTO.getSupplier_id(),
                supperOrderDTO.getDate(),
                supperOrderDTO.getTime(),
                supperOrderDTO.getTotal()
        );
    }
    ...
    private static List<SupperOrderTm> toTms(ResultSet resultSet) throws SQLException {
    ...
    public static List<SupperOrderTm> findCustomerOrdersByDate(String date) throws SQLException, ClassNotFoundException {
    ...
    public static List<ReportCm> getSupplierOrder() throws SQLException, ClassNotFoundException {
    ...
    public static List<ReportCm> getYearlySupplierOrder(String year) throws SQLException, ClassNotFoundException {
    ...
    public static List<ReportCm> getMonthlySupplierOrder(String year, String moth) throws SQLException, ClassNotFoundException {
        String[] allMonth = {"JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"};
        int month=0;
    ...
    public static List<ReportCm> getDaySupplierOrder(String year, String month) throws SQLException, ClassNotFoundException {
        String[] allMonth = {"JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"};
        int thisMonth=0;
        for (int i = 0; i < allMonth.length; i++) {
